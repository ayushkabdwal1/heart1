...
const createAppointment = async (
  payload: any
): Promise<Appointments | null | any> => {
  console.log("Inside Appointement");
  console.log("payloadofAppointment", payload);
  const { patientInfo, payment } = payload;

  if (patientInfo.patientId && patientInfo.patientId!==null) {
      console.log("patientInfo", patientInfo.patientId);
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    console.log("Doctor can not exist");
    // console.log("RESultttttttttttt", result);
    const appointmentLastNumber = (previousAppointment?.trackingId ?? "").slice(
      -3
    );
    const lastDigit = (Number(appointmentLastNumber) + 1 || 0)
      .toString()
      .padStart(3, "0");
    ...
    const trackingId =
      first3DigitName + year + month + day + lastDigit || "001";
...
const createAppointmentByUnAuthenticateUser = async (
  payload: any
): Promise<Appointments | null> => {
  const { patientInfo, payment } = payload;
  console.log("patientInfo", payload);
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const lastDigit = (Number(appointmentLastNumber) + 1)
      .toString()
      .padStart(3, "0");
...
const getAppointmentByTrackingId = async (
  data: any
): Promise<Appointments | null> => {
  const { id } = data;
...
//doctor Side
const getDoctorAppointmentsById = async (
  user: any,
  filter: any
): Promise<Appointments[] | null> => {
  const { userId } = user;
...
const updateAppointmentByDoctor = async (
  user: any,
  payload: Partial<Appointments>
): Promise<Appointments | null> => {
  const { userId } = user;
